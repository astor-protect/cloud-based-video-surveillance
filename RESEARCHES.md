## my travis github status show build unknown but build is Passing

https://www.google.com/search?q=my+travis+github+status+show+build+unknown+but+build+is+passing&oq=my+travis+github+status+show+build+unknown+but+build+is+passing&aqs=chrome..69i57j33l2.46512j0j7&sourceid=chrome&ie=UTF-8

https://stackoverflow.com/questions/12030304/why-is-my-travis-ci-build-status-always-reflected-as-unknown

## alpine linux

https://wiki.alpinelinux.org/wiki/Alpine_Linux_package_management

## spring boot failed to deploy to heroku with travis

https://www.google.com/search?q=spring+boot+failed+to+deploy+to+heroku+with+travis&oq=spring+boot+failed+to+deploy+to+heroku+with+travis&aqs=chrome..69i57.104282j0j7&sourceid=chrome&ie=UTF-8

https://dzone.com/articles/spring-boot-heroku-and-cicd

https://devcenter.heroku.com/articles/deploying-java-applications-with-the-heroku-maven-plugin

https://github.com/gabrielfeitosa/ci-spring-boot/blob/master/.travis.yml

https://github.com/nitesh009/spring-boot-app

## heroku maven plugin

https://www.google.com/search?q=heroku+maven+plugin&oq=heroku+maven+&aqs=chrome.1.69i57j0l2j69i60l3.10785j0j7&sourceid=chrome&ie=UTF-8

https://github.com/heroku/heroku-maven-plugin

https://devcenter.heroku.com/articles/logging#view-logs

## how to select app using heroku cli

https://www.google.com/search?q=how+to+select+app+using+heroku+cli&oq=how+to+select+app+using+heroku+cli&aqs=chrome..69i57j33l3.47321j0j7&sourceid=chrome&ie=UTF-8

https://devcenter.heroku.com/articles/heroku-cli-commands

## web process failed to bind to $port within 90 seconds of launch spring boot

https://www.google.com/search?sxsrf=ACYBGNQxSNN1GN-bzI6jdkrwijDsVcEZ7g%3A1568384892609&ei=fKd7XafhJIyAhbIP5ry3qA8&q=web+process+failed+to+bind+to+%24port+within+90+seconds+of+launch+spring+boot+&oq=web+process+failed+to+bind+to+%24port+within+90+seconds+of+launch+spring+boot+&gs_l=psy-ab.3...14223.27933..28769...2.2..3.1922.12111.3-1j1j1j3j5j1......0....1..gws-wiz.......0i71j33i10.SuEdm1-gGYQ&ved=0ahUKEwinj5SDgc7kAhUMQEEAHWbeDfUQ4dUDCAo&uact=5

https://stackoverflow.com/questions/45557881/spring-boot-on-heroku-web-process-failed-to-bind-to-port-within-90-seconds-of

https://docs.spring.io/spring-boot/docs/current/reference/htmlsingle/#howto-change-the-http-port

## github site maven plugin with travis

https://www.google.com/search?q=github+site+maven+plugin+with+travis&oq=github+site+maven+plugin+with+travis&aqs=chrome..69i57j69i64.31794j0j7&sourceid=chrome&ie=UTF-8

https://blog.lanyonm.org/articles/2015/12/19/publish-maven-site-github-pages-travis-ci.html

https://help.github.com/en/articles/creating-a-personal-access-token-for-the-command-line

https://github.com/lanyonm/playground
